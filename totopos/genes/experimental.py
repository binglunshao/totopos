...
def topological_gene_scores_via_perturbation_ripser(
    ...
    dists = differentiable_distance_matrix_torch(pcs_np[rep_indices] if n_pts < adata.n_obs else pcs_np)
...
def topological_gene_scores_via_perturbation_ripser_spf(
    ...
    dists = differentiable_distance_matrix_torch(pcs_np[ph["idx_perm"]] if n_pts < adata.n_obs else pcs_np)
